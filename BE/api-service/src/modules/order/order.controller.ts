...
export class OrderController {
  ...
  @Get('/:id/payment')
  @ApiOperation({
    summary: 'Get order payment url',
  })
  @AuthorizeGuard([USER_ROLE.USER])
  @HttpCode(HttpStatus.OK)
  async getOrderPaymentUrl(
    @Request() req: any,
    @Param('id', ParseIntPipe) orderId: number,
  ) {
    const data = await this.orderService.getOrderPaymentUrl(
      orderId,
      req.user.id,
    );
    return ValidHttpResponse.toOkResponse(data);
  ...
  async updateStatus(@Param('id') id, @Body() updateOrderDto: UpdateOrderDto) {
  ...
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'User received order',
  })
  async received(@Req() req, @Param('id') orderId) {
    await this.orderService.received(req.user.id, orderId);
    return ValidHttpResponse.toNoContentResponse();
  }
