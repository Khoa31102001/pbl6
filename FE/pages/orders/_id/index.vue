<template>
  <b-form class="d-flex flex-column" @submit.prevent="checkout">
    ...
    >
      ...
      <b-button
        v-if="order.status === orderStatus.ON_DELIVERY"
        variant="outline-primary"
        style="width: 150px"
        @click="received"
      >
...
export default Vue.extend({
  ...
  methods: {
    ...
    checkout() {
      this.$axios
        .$get(`/orders/${this.order.id}/payment`)
        .then((res) => {
        ...
        .catch((error) => {
          if (error.statusCode === 400) {
            ...
            const sizeName = this.order.orderItems.find(
    ...
    received() {
      this.$axios
        .$put(`/orders/${this.order.id}/received`)
        .then(() => {
          this.$bvToast.toast(this.$t('orders.received.success') as string, {
            title: this.$t('toastTitle.success') as string,
            variant: 'success',
          });
          this.$nuxt.refresh();
        ...
        .catch(() => {
          this.$bvToast.toast(this.$t('orders.received.error') as string, {
            title: this.$t('toastTitle.error') as string,
            variant: 'danger',
          });
        });
